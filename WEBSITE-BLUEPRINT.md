...
- [ ] **Reflow bei 200 % und 400 % Zoom prüfen** (`zoom.mjs`/`zoom3.mjs`; 400 % entspricht 320px effektiver Breite, das ist der WCAG-1.4.10-Prüffall). Die Seite darf nicht horizontal scrollen **und kein Inhalt verloren gehen** — beides prüfen, denn ein `overflow: hidden` verhindert das Scrollen und schneidet den Überstand still ab.
- [ ] **Abgeschnittenen Text separat messen** (`clip.mjs`): je Textknoten `scrollWidth` gegen die rechte Kante des nächsten Vorfahren mit `overflow-x != visible`. „Kein horizontales Scrollen" ist als alleiniges Kriterium wertlos, sobald ein Container `overflow-x: hidden` trägt — genau dann scrollt nichts und der Text ist trotzdem weg (C63).
- [ ] **Lange Komposita in Überschriften brechen lassen.** Deutsche Wörter wie „Datenschutzerklärung" (316px bei 32px Schriftgröße) passen ab etwa 360px Viewport nicht mehr in die Spalte — das trifft normale Telefone, nicht nur den Zoom-Prüffall. `overflow-wrap: break-word` als Netz, `hyphens: auto` für die schöne Trennung, **und ein weiches Trennzeichen `&shy;` an der fachlich richtigen Stelle**: ohne Trennwörterbuch (headless Chromium, manche Linux-Builds) bleibt `hyphens: auto` wirkungslos und der Browser trennt mitten im Wort ohne Bindestrich. `&shy;` ist unsichtbar, ändert den Wortlaut nicht und wird beim Kopieren weggelassen (C63).
